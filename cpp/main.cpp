...
int main()
{
    ...
    auto handleKeys = [&](WPARAM key)
    {
        ...
        // Prüfen, ob eines deiner Fenster aktiv ist
        if (foreground != winRay.getHWND() &&
            foreground != winTop.getHWND() &&
            foreground != winFront.getHWND() &&
            foreground != winSide.getHWND() &&
            foreground != winPersp.getHWND())
        {
            return;
        }
