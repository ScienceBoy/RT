...
std::map<std::string, MaterialInfo> loadMTL(const std::string& filename) 
{
    ...
    while(std::getline(file, line)) {
        ...
        if(token == "newmtl") {
            ...
            double ks_avg = (r + g + b) / 3.0;

            mat.specular = Farbe(r,g,b);
            mat.specularStrength = ks_avg;

            // Reflection nur schwach ableiten
            mat.reflection = ks_avg * 0.2;

        } else if(token == "Ke") {
            double r,g,b;
            iss >> r >> g >> b;

            mat.emission = Farbe(r,g,b);
        } else if(token == "Ns") {
            double s; iss >> s;
            //mat.shininess = 2.0 + (s / 1000.0) * 126.0;
            mat.shininess = s;
            double ns = std::clamp(s, 0.0, 1000.0);
            mat.roughness = sqrt(2.0 / (ns + 2.0));
        } else if(token == "d") {
            iss >> mat.alpha;
        } else if(token == "Tr") {
            iss >> mat.alpha;
        ...
        if(type == "v") { // Vertex
            ...
            if(!materials.empty()){
                ...
                matHere = Material(
                    ...
                    currentMat.specularStrength,
                    currentMat.reflection,
                    currentMat.roughness,

                    currentMat.IndexOfRefraction,
                    1.0 - currentMat.alpha,
    ...
    const Material& defaultMat)
{
    ...
    while (ptr < end) {
        ...
        // --- Normal ---
        else if (p[0] == 'v' && p[1] == 'n' && std::isspace(p[2])) {
            p += 3;
            skipSpaces(p);
            double x = parsedouble(p);
            skipSpaces(p);
        ...
        // --- Texture Coord (UV) ---
        else if (p[0] == 'v' && p[1] == 't' && std::isspace(p[2])) {
            p += 3;
            skipSpaces(p);
